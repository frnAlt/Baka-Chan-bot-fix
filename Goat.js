                        ...
                        setTimeout(() => {
                                try {
                                        ...
                                        try {
                                                ...
                                                // file temporarily inaccessible, skip timestamp update
                                        }
                                }
                        }, 200);
                }
...
class MemoryManager {
        constructor(options = {}) {
                this.options = {
                        checkInterval: options.checkInterval || 3 * 60 * 1000, // 3 minutes (more frequent on Render)
                        heapThreshold: options.heapThreshold || 350 * 1024 * 1024, // 350MB — triggers well before Render's 512MB OOM
        ...
        _performCleanup() {
                ...
                // Cleanup receivedTheFirstMessage — grows unbounded, cap at 5000 entries
...
(async () => {
        ...
        try {
                ...
                log.warn("VERSION CHECK", `Skipped — could not reach GitHub: ${err.message}`);
        }
        // ———————————————————— LOGIN ———————————————————— //

// @ts-check
const {  CassEXP  } = require("../modules/cassEXP.js");
const {  clamp, countEmojis, UNISpectra  } = require("@cassidy/unispectra");
const {  Inventory, Collectibles  } = require("@cass-modules/InventoryEnhanced");
const {  PetPlayer  } = require("./pet-fight");
const { 
  abbreviateNumber,
  formatCash,
  formatValue,
  parseBet,
 } = require("@cass-modules/ArielUtils");
const {  calculateInflation  } = require("@cass-modules/unitypes");
const {  Slicer  } = require("./utils-liane");
 ...
 * @returns
 */
function generateGift(type = "gift", assign = {}) {
  if (type === "pack") {
...
function generateChequeGift(amount = 100, groups = []) {
  return {
    key: `cheque_${amount}`,
    icon: "💵",
...
const treasureInv = new Inventory(treasures, Infinity);

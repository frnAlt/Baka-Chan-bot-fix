// @ts-check
const {  forceTitleFormat  } = require("cassidy-styler");
const { 
  UNIRedux,
  UNISpectra,
  emojiEnd,
 } = require("../../CommandFiles/modules/unisym.js");

const FontSystem = require("./fonts");
 ...
 * @returns
 */
function parseTemplate(text, ...replacers) {
  text = String(text);
  try {
 ...
 * @returns
 */
function styled(text = "", StyleClass) {
...
function makeFieldsEnumerable(instance) {
...
function applyText(text, styling) {
...
function applyLine(text, styling) {
  text = String(text); //.trimStart().trimEnd();
...
function styledForHTML(text = "", StyleClass) {
